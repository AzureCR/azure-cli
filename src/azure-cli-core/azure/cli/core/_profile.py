...
class Profile(object):
    ...
    def get_refresh_credentials(self, resource=CLOUD.endpoints.active_directory_resource_id,
                                subscription_id=None):
        account = self.get_subscription(subscription_id)
        user_type = account[_USER_ENTITY][_USER_TYPE]
        username_or_sp_id = account[_USER_ENTITY][_USER_NAME]
        ...
        if user_type == _USER:
            refresh_token = self._creds_cache.retrieve_token_entry_for_user(
                username_or_sp_id, account[_TENANT_ID], resource)[_REFRESH_TOKEN]
            return None, refresh_token, account[_TENANT_ID]
        else:
            sp_secret = self._creds_cache \
                .retrieve_secret_of_service_principal(username_or_sp_id)
            return username_or_sp_id, sp_secret, account[_TENANT_ID]
