    ...
    for i in range(0, retry_times):
        ...
        try:
            ...
            if response.status_code in [200, 202, 204]:
                result, next_link = None, None
                try:
                    result = response.json()[result_index] if result_index else response.json()
                except ValueError:
                    logger.debug('The response is empty or is not in json format.')
...
def acr_repository_show_attributes(cmd,
                                   registry_name,
                                   repository=None,
                                   image=None,
                                   resource_group_name=None,
...
def acr_repository_update_attributes(cmd,
                                     registry_name,
                                     repository=None,
                                     image=None,
                                     resource_group_name=None,
...
def _validate_parameters(repository, image):
    if bool(repository) == bool(image):
        raise CLIError('Usage error: --image [REPOSITORY:TAG|REPOSITORY@DIGEST] | --repository REPOSITORY')
